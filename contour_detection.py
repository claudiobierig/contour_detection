...
def save_image(image, image_path, output_dir, parameter):
    ...
    cv2.imwrite(
        os.path.join(
            output_dir, f"{input_file_name_without_extension}_{parameter}{input_file_extension}"
        ),
        image
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", help="Folder in which to save the converted images.", default="out")
    ...
    for file, parameter in itertools.product(arguments.input_files, arguments.parameters):
        ...
        save_image(image, file, arguments.output_dir, parameter)
